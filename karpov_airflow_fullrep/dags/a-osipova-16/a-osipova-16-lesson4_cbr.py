...
dag = DAG("a-osipova-16-lesson4_cbr", # Меняем название нашего DAG
...
load_cbr_xml_script = '''
curl https://www.cbr.ru/scripts/XML_daily.asp?date_req=21/01/2022 | iconv -f Windows-1251 -t UTF-8 > /tmp/a-osipova-16-lesson4_cbr.xml
'''
...
def export_xml_to_csv_func():
    ...
    tree = ET.parse('/tmp/a-osipova-16-lesson4_cbr.xml', parser=parser)
    root = tree.getroot() # Корень нашего файла

    with open('/tmp/a-osipova-16_cbr.csv', 'w') as csv_file: # Открываем csv в которую будем писать построчно каждый элемент, который нас интересует: Valute, NumCode и т.д.
...
def load_csv_to_gp_func():
    pg_hook = PostgresHook(postgres_conn_id='conn_greenplum') # Создаем hook, записываем наш гринплан
    # TODO: Мёрдж или очищение предыдущего батча
    pg_hook.copy_expert("COPY a_osipova_16_lesson4_cbr FROM STDIN DELIMITER ','", '/tmp/a-osipova-16_cbr.csv')
