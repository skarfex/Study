...
with DAG("a_tarhova_lesson_4_practice",
    ...
    #week_without_Sunday = ShortCircuitOperator(
    #    task_id='week_without_Sunday',
    #    python_callable=is_week_without_Sunday_func,
    ...
    def load_greenplum_articles_func():
        ...
        cursor = conn.cursor()
        sql = "SELECT heading FROM articles WHERE id = :week_day"
        w_d = is_week_without_Sunday_func('{{ ds }}')
        cursor.execute(sql, {"week_day": w_d})
