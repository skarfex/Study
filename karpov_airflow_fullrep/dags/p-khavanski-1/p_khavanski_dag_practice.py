...
load_cbr_xml_script = '''
curl https://www.cbr.ru/scripts/XML_daily.asp?date_req=11/01/2022 | iconv -f Windows-1251 -t UTF-8 > /tmp/p_khavanski_cbr.xml
'''
...
def export_xml_to_csv_func():
    ...
    tree = ET.parse('/tmp/p_khavanski_cbr.xml', parser=parser)
    root = tree.getroot()

    with open('/tmp/p_khavanski_cbr', 'w') as csv_file:
...
def load_csv_to_gp_func():
    ...
    pg_hook.copy_expert("COPY kolesnik_cbr FROM STDIN DELIMITER ','", '/tmp/p_khavanski_cbr.csv')
