...
Airflow DAG. Lesson 4. Practice session
"""
...
XML_FILE = "/tmp/s_gerasimov_17_cbr.xml"
CSV_FILE = "/tmp/s_gerasimov_17_cbr.csv"
CBR_URL = "https://www.cbr.ru/scripts/XML_daily.asp?date_req=01/11/2021"

dag = DAG("s-gerasimov-17-load-cbr",  # Меняем название нашего DAG
...
load_cbr_xml_script = "curl " + CBR_URL + " | iconv -f Windows-1251 -t UTF-8 > " + XML_FILE
...
def export_xml_to_csv_func():
    parser = ET.XMLParser(
        encoding="UTF-8")  # Создаем Parser формата UTF-8, натравливаем его на нужный файл ('/tmp/s_gerasimov_17_cbr.xml', parser=parser)
