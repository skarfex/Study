import datetime as dt
print (dt.date.today())

date = dt.date(2022,5,8)
print(dt.datetime.weekday(dt.date.today()))

print(dt.datetime.weekday(date))

print(dt.datetime.isoweekday(date))

# Подключение к GP

import psycopg2

conn = psycopg2.connect(dbname='karpovcourses',
                       user='student',
                       password='Wrhy96_09iPcreqAS',
                       host='greenplum.lab.karpov.courses',
                       port=6432)
cursor = conn.cursor()

week_day_num = 6

cursor.execute(f'SELECT heading FROM articles WHERE id = {week_day_num}')  # исполняем sql
query_res = cursor.fetchall()[0]  # полный результат
# one_string = cursor.fetchone()[0]  # если вернулось единственное значение

print (query_res)


"""
dag with custom operator 3 (Airflow, les 5)
"""
import logging

import psycopg2
import requests
from airflow import AirflowException
from airflow.hooks.postgres_hook import PostgresHook
from airflow.models import BaseOperator

url = 'https://rickandmortyapi.com/api/location'

class E_Golovenkin_20_Get_Top_locations (BaseOperator):
    '''
    Get top 3 locations by number of residents from API
    and write data to DB
    '''

    # template_fields = () # It is place for your variables
    ui_colour = '#e0ffff' # colour of your operator on the interface Airflow

    def __init__ (self, **kwargs) -> None:
        super().__init__(**kwargs)

    def top_3_locations (url = 'https://rickandmortyapi.com/api/location'):
        '''
        Get top 3 locations by number of residents from API
        :param url: 'https://rickandmortyapi.com/api/location'
        :return: max_id_loc (list of id op 3 locations)
        '''
        r = requests.get(url=url)
        if r.status_code == 200:
            data = r.json().get('results') # выбираем основной блок с данными
            loc_num = len(data)-1 # находим общее количество локаций
            stat_dict = {} # пустой словарь для статистики
            x = 0 # инкремент для цикла
            while x != loc_num:
                loc = data[x]
                id = loc.get("id")
                res_cnt = loc.get('residents')
                stat_dict[id] = len(res_cnt)
                x += 1
            logging.info(f'{stat_dict}') # выводим всю статистику
            max_id_loc = sorted(stat_dict, key=stat_dict.get, reverse=True)[:3]
            logging.info(f'{max_id_loc}') # топ 3 локации
            return max_id_loc
        else:
            logging.warning(f'HTTP STATUS {r.status_code}')
            raise AirflowException('Error in load page count')

    def execute (self, context):
        '''
        Get top 3 locations by number of residents
        rom API and write data to DB
        '''
        loc_id = self.top_3_locations()
        pg_hook = PostgresHook(postgres_conn_id='conn_greenplum_write')
        for i in loc_id:
            loc_info = requests.get(url_loc=f'https://rickandmortyapi.com/api/location/{i}').json()
            query = f'''
            insert into e_golovenkin_20_ram_location
            (id, name, type, dimension, resident_cnt)
            values (
                    {loc_info.get('id')}, '{loc_info.get('name')}',
                   '{loc_info.get('type')}', '{loc_info.get('dimension')}',
                    {len(loc_info.get('residents'))}
                    )
                    '''
            logging.info(f'{query}')
            try:
                pg_hook.run(query, False)
            except (psycopg2.Error) as error:
                print("Error while inserting", error)





    def top_3_location (url = 'https://rickandmortyapi.com/api/location'):
        '''
        Get top 3 locations by number of residents from API
        :param url: 'https://rickandmortyapi.com/api/location'
        :return: max_id_loc (list of id op 3 locations)
        '''
        r = requests.get(url=url)
        if r.status_code == 200:
            data = r.json().get('results') # выбираем основной блок с данными
            loc_num = len(data)-1 # находим общее количество локаций
            stat_dict = {} # пустой словарь для статистики
            x = 0 # инкремент для цикла
            while x != loc_num:
                loc = data[x]
                id = loc.get("id")
                res_cnt = loc.get('residents')
                stat_dict[id] = len(res_cnt)
                x += 1
            logging.info(f'{stat_dict}') # выводим всю статистику
            max_id_loc = sorted(stat_dict, key=stat_dict.get, reverse=True)[:3]
            logging.info(f'{max_id_loc}') # топ 3 локации
            return max_id_loc
        else:
            logging.warning(f'HTTP STATUS {r.status_code}')
            raise AirflowException('Error in load page count')

    # loc_id = top_3_location(url)
    # print (loc_id)

    def write_data_to_db (self):
        loc_id = self.top_3_location()
        pg_hook = PostgresHook(postgres_conn_id='conn_greenplum_write')
        for i in loc_id:
            loc_info = requests.get(url=f'https://rickandmortyapi.com/api/location/{i}').json()
            query = f'''
            insert into e_golovenkin_20_ram_location
            (id, name, type, dimension, resident_cnt)
            values (
                    {loc_info.get('id')}, '{loc_info.get('name')}',
                   '{loc_info.get('type')}', '{loc_info.get('dimension')}',
                    {len(loc_info.get('residents'))}
                    )
                    '''
            logging.info(f'{query}')
            try:
                pg_hook.run(query, False)
            except (psycopg2.Error) as error:
                print("Error while inserting", error)
#
# # ---------------------------------------------------
# import psycopg2
#
# conn = psycopg2.connect(dbname='students',
#                         user='student',
#                         password='Wrhy96_09iPcreqAS',
#                         host='greenplum.lab.karpov.courses',
#                         port=6432)
# conn.autocommit = True
# cursor = conn.cursor()
#
# for i in loc_id:
#     loc_info = requests.get(url=f'https://rickandmortyapi.com/api/location/{i}').json()
#     try:
#         query = f'''
#         insert into e_golovenkin_20_ram_location
#         (id, name, type, dimension, resident_cnt)
#         values ({loc_info.get('id')}, '{loc_info.get('name')}',
#         '{loc_info.get('type')}', '{loc_info.get('dimension')}',
#         {len(loc_info.get('residents'))})
#         '''
#         print (query)
#
#         cursor.execute(query)
#     except (psycopg2.Error) as error:
#         print("Error while insert to Postgres SQL", error)

# print (len(data))
# ex = stat[3]
# l = ex.get('residents')
# print (ex.get("id"))
# print (len(l))

# r = requests.get(url=url)
# data = r.json().get('results')
#
# stat_dict = {}
# total = len(data)-1
# x = 0
# while x!=total:
#         loc = data[x]
#         id = loc.get("id")
#         res_cnt = loc.get('residents')
#         stat_dict [id] = len(res_cnt)
#         x+=1
#
# print (stat_dict)
#
# max_id_loc = sorted(stat_dict, key=stat_dict.get, reverse=True)[:3]
# print (max_id_loc) # [3, 6, 1]

# max_id_loc = sorted(stat_dict, key=stat_dict.get, reverse=True)[:3]
# print (max_loc)

