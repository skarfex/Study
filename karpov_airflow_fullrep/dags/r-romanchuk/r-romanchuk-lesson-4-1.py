...
# TODO: удалять файл r_romanchuk_cbr.xml, если он уже есть
load_cbr_xml_script = '''
curl https://www.cbr.ru/scripts/XML_daily.asp?date_req=01/11/2021 | iconv -f Windows-1251 -t UTF-8 > /tmp/r_romanchuk_cbr.xml
'''
...
def export_xml_to_csv_func():
    ...
    tree = ET.parse('/tmp/r_romanchuk_cbr.xml', parser=parser)
    root = tree.getroot()

    with open('/tmp/r_romanchuk_cbr.csv', 'w') as csv_file:
...
def load_csv_to_gp_func():
    ...
    pg_hook.copy_expert("COPY r_romanchuk_cbr FROM STDIN DELIMITER ','", '/tmp/r_romanchuk_cbr.csv')
