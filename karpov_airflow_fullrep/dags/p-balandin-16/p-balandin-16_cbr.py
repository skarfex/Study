...
dag = DAG("p-balandin-16_cbr", # Меняем название нашего DAG
...
load_cbr_xml_script = F'curl https://www.cbr.ru/scripts/XML_daily.asp?date_req=01/11/2021 | iconv -f Windows-1251 -t UTF-8 > C:\Users\james\Downloads\DE - Karpov\cbr.xml'
...
def export_xml_to_csv_func():
    ...
    tree = ET.parse('C:\Users\james\Downloads\DE - Karpov\cbr.xml', parser=parser)
    root = tree.getroot() # Корень нашего файла

    with open('C:\Users\james\Downloads\DE - Karpov\cbr.csv', 'w') as csv_file: # Открываем csv в которую будем писать построчно каждый элемент, который нас интересует: Valute, NumCode и т.д.
...
def load_csv_to_gp_func():
    ...
    pg_hook.copy_expert("COPY load_cbr FROM STDIN DELIMITER ','", 'C:\Users\james\Downloads\DE - Karpov\cbr.xml')
