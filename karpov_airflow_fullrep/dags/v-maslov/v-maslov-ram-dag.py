"""
Даг для чтения данных по локациям с Rick and Morty API
"""
...
from v_maslov_plugins.v_maslov_top_locations_operator import VMaslovRamTopLocationsOperator
...
with DAG("v_maslov_ram",
    ...
    def load_top3_locations_to_gp_func(**kwargs):
        ...
        cursor = conn.cursor()
        cursor.execute('''
            CREATE TABLE if not exists public.v_maslov_ram_location (
                id int4 NOT NULL,
                "name" varchar NULL,
                "type" varchar NULL,
                dimension varchar NULL,
                resident_cnt int4 NULL,
                CONSTRAINT v_maslov_ram_location_pkey PRIMARY KEY (id)
            )
            DISTRIBUTED BY (id);
        ''')

        cursor.execute('TRUNCATE TABLE public.v_maslov_ram_location')
        ...
        cursor.execute(f'''
            INSERT INTO public.v_maslov_ram_location(id, name, type, dimension, resident_cnt)
            VALUES
            {rows}
        ''')
    ...
    extract_top3_locations_2 = VMaslovRamTopLocationsOperator(
