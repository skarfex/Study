...
with DAG("n-kutsepalov-lesson-2", # название такое же, как и у файла для удобной ориентации в репозитории
    ...
    def hello_world_func():
        logging.info("Hello DAG!")

    def text_test_func():
        logging.info("Тестовый ДАГ")
    ...
    text_test = PythonOperator(
        task_id='text_test',
        python_callable=text_test_func,  # ссылка на функцию, выполняемую в рамках таски
        dag=dag
    )

    start >> [echo_ds, hello_world] >> text_test