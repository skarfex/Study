...
with DAG (
    "s-dobrynin-dag3",
    schedule_interval='0 0 * * 1-6', #в 12 ночи с пон по субботу, https://crontab.guru/#0_0_*_*_1-6
    ...
    def connect_func(**kwargs):
        ...
        day_of_week = datetime.strptime(kwargs['ds'], '%Y-%m-%d').weekday()+1 #номер дня недели, 1 понедельник
    ...
    print_args = PythonOperator(
        ...
        templates_dict = {'kwarg1': '{{ti.xcom_pull(task_ids = "connect_operator", key="return_value")}}'} ,#Возвращаем результат xcomm, читает. Второй способ Jinja
