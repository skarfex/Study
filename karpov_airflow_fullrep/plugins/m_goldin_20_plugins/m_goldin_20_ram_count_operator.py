...
class MGOLDIN20RAMCOUNTOPERATOR (BaseOperator):
    ...
    def execute(self, context):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute(f'CREATE TABLE IF NOT EXISTS m_goldin_20_ram_location(id int , name varchar, type varchar, dimension varchar, resident_cnt int)')
            logging.info(f'list field all {l2}')
            logging.info(f'INSERT INTO m_goldin_20_ram_location values({l2[0]},\'{l2[1]}\',\'{l2[2]}\',\'{l2[3]}\',{l2[4]})')
            cursor.execute(f'INSERT INTO m_goldin_20_ram_location values({l2[0]},\'{l2[1]}\',\'{l2[2]}\',\'{l2[3]}\',{l2[4]})')
            conn.commit()
        except Exception:
            logging.info(f'id {l2[0]} is not unique in tables m_goldin_20_ram_location')
        conn.close()
