...
class ASokolovRickMortyHook(HttpHook):
    """
...
class ASokolovGetTopLocationOperator(BaseOperator):
    """
        ...
        with pg_hook.get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute(f"select * from information_schema.tables t where t.table_name = "
                            f"'a_sokolov_6_ram_location';")
                if cur.fetchone()[0] == 0:
                    cur.execute('CREATE TABLE public.a_sokolov_6_ram_location(id text,name text,type text,'
    ...
    def load_csv_to_gp_func(self):
        ...
        with pg_hook.get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute(f"truncate a_sokolov_6_ram_location;")
        pg_hook.copy_expert("COPY a_sokolov_6_ram_location FROM STDIN DELIMITER ','", '/tmp/a_sokolov_6_ram_toploc'
                                                                                       '.csv')
        conn.close()
    ...
    def execute(self, context):
        ...
        hook = ASokolovRickMortyHook('dina_ram')
        ...
        df.to_csv('/tmp/a_sokolov_6_ram_toploc.csv', sep=',', header=False, index=False)
