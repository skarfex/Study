...
class IlKuznetsovOperator(BaseOperator):
    ui_color = "#e0ffff"
    ...
    def get_page_count(self):
        r = requests.get(IlKuznetsovOperator.base_url)
    ...
    def get_location_info_on_page(self,page_id):
        r = requests.get(IlKuznetsovOperator.page_url.format(pg=page_id))
    ...
    def execute(self, context, **kwargs):
        ...
        for res in result:
            SQL = f"""INSERT INTO il_kuznetsov_ram_location
            SELECT {res[0]} as id, '{res[1]}' as name, '{res[2]}' as type, '{res[3]}' as dimension, {res[4]} as resident_cnt
            where NOT EXISTS(SELECT * from public.il_kuznetsov_ram_location where id = {res[0]})"""
