...
class NChumurovRamTopResidentLocationsOperator(BaseOperator):
    ...
    ui_color = "#a256c4"
    logging.info('начало класса')

    def __init__(self, top_size: int = 3, **kwargs) -> None:
        super().__init__(**kwargs)
        self.top_size = top_size
        logging.info('инит прошел')

    def execute(self, context):
        logging.info('до хука')
        hook = N_ChumurovRamHook('dina_ram')
        logging.info('хук прошел')
        locations = []
        ...
        result =  sorted(locations, key=lambda x: x['resident_cnt'], reverse=True)[:self.top_size]
