...
class EMerkusheva19ramtoplocationoperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        try:
            cursor.execute(
                f'CREATE TABLE IF NOT EXISTS e_merkusheva_19_ram_location(id int UNIQUE, name varchar, type varchar, dimension varchar, resident_cnt int)')
            logging.info(f'list field all {l2}')
            logging.info(
                f'INSERT INTO e_merkusheva_19_ram_location values({l2[0]},\'{l2[1]}\',\'{l2[2]}\',\'{l2[3]}\',{l2[4]})')
            cursor.execute(
                f'INSERT INTO e_merkusheva_19_ram_location values({l2[0]},\'{l2[1]}\',\'{l2[2]}\',\'{l2[3]}\',{l2[4]})')
            conn.commit()
        except Exception:
            logging.info(f'id {l2[0]} is not unique in tables e_merkusheva_19_ram_location')
        conn.close()
